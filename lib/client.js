...
class WasmFaasClient {
  ...
  async __processMessage(msg) {
    ...
    switch(msg.type) {
      case "invoke":
        this.logger("Invoke request received, function: %s, id: %s", msg.name, msg.request_id);
        ...
        this.logger("Replying to request: %s", msg.request_id);
        return reply;
  ...
  start() {
    ...
    ws.on('open', async () => {
      this.logger("WS to %s initiated", this.wsUri);
    })
    ...
    ws.on('close', async (data) => {
      ...
      this.logger("WS server terminated connection %d", data);
    });
